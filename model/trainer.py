...
class expressionDataset(Dataset):
    def __init__(self, x, y, isTrain, samples_weight, src_len=512, new_index=None):
        self.x = x
        self.y = y
    ...
    def __getitem__(self, idx):
        if self.isTrain:
            new_idx = self.new_index[idx]
            output_x = np.stack(self.x[new_idx:(new_idx+self.src_len)])
            output_y = np.stack(self.y[new_idx:(new_idx+self.src_len)])
            weight = np.stack(self.samples_weight[new_idx:(new_idx+self.src_len)])
        else:
...
def getDataloader(x, y, isTrain, batch_size, window_length, samples_weight, new_index=None):
    if isTrain:
        dataset = expressionDataset(x, y, True, samples_weight, window_length, new_index)
        dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=True)
    else:
        dataset = expressionDataset(x, y, False, samples_weight, window_length, new_index)
