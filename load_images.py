...
def sanityCheck(dataset_name):
    if (dataset_name == 'CASME_sq'):
        # Save the images into folder 'rawpic_crop'
        graphDataPath = 'graphData_k6_phase_openface/'
        for subjectName in glob.glob(dataset_name + '/rawpic_crop_aligned_openface/*'):
            # Create new directory for each subject
            dir_crop_sub = dataset_name + '/rawpic_crop_aligned_openface/' + str(subjectName.split('/')[-1])

            print('Subject', subjectName.split('/')[-1])
            #print(len(glob.glob(dir_crop_sub+'/*')))
            for vid in glob.glob(dir_crop_sub+'/*'):
                dir_crop_sub_vid = dir_crop_sub + '/'+ vid.split('/')[-1]  # Get dir of video
                length = len(glob.glob(vid + '/' + vid.split('/')[-1] + '_aligned/frame*.bmp'))
                #print(len(glob.glob(vid + '/' + vid.split('/')[-1] + '_aligned/frame*.bmp')))
                datapath = 'CASME_sq/' + graphDataPath + dir_crop_sub_vid.split('/')[-1] + '_node.npy'
                data = np.load(datapath)
                #print(data.shape)
                if length != data.shape[3]:
                    print('WRONG:' + str(datapath))
    elif (dataset_name == 'SAMMLV'):
        graphDataPath = 'graphData_k37_phase_openface_level2/'
        print(graphDataPath)
        for vid in glob.glob(dataset_name + '/SAMM_longvideos_crop_aligned_openface/*'):
            #print('Video', vid.split('/')[-1])
            length = len(glob.glob(vid + '/' + vid.split('/')[-1] + '_aligned/frame*.bmp'))
            datapath = 'SAMMLV/' + graphDataPath + vid.split('/')[-1] + '_node.npy'
            #print(datapath)
            try:
                data = np.load(datapath)
            except:
                print('ERROR LOADING:'+str(datapath))
            # print(data.shape)
            if length != data.shape[3]:
                print('WRONG:' + str(datapath))
